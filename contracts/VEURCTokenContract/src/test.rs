...
#[cfg(test)]
mod tests {
    use crate::v_eurc::{VEURCToken, VEURCTokenClient};
    ...
    fn test_initialize() {
        let env = Env::default();
        let contract_id = env.register(VEURCToken, ());
        let client = VEURCTokenClient::new(&env, &contract_id);
        ...
        client.initialize(
            &admin,
            &7u32,
            &"Vault EURC".into_val(&env),
            &"vEURC".into_val(&env),
        );

        assert_eq!(client.admin(), admin);
        assert_eq!(client.decimals(), 7u32);
        assert_eq!(client.name(), "Vault EURC".into_val(&env));
        assert_eq!(client.symbol(), "vEURC".into_val(&env));
